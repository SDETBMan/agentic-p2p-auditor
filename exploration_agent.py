"""
Domain-agnostic exploration loop for end-to-end QA using the Anthropic Messages API.
All domain-specific logic (store, tools, prompts) is injected via DomainSpec.
"""
...
def run_exploration(
    ...
    *,
    ...
    """Run the exploration agent loop for the given domain.

    Args:
        domain: The DomainSpec providing tools, prompts, and store class.
        store: Pre-created mock store instance. If None, one is created.
        ...
        live: If True, dispatch tool calls via domain's live HTTP adapter.
    """
    if store is None:
    ...
    initial = user_prompt or domain.default_exploration_user_prompt
    ...
    for iteration in range(max_iterations):
        ...
        try:
            response = client.messages.create(
                ...
                max_tokens=8192,
                temperature=0,
                system=domain.exploration_system_prompt,
                messages=messages,
                tools=tools,
            )
        ...
        if not tool_uses:
            ...
            if iteration >= 19:
                ...
                nudge_body = (
                    "Continue using tools until the workflow is finished. "
        ...
        for block in tool_uses:
            ...
            # Dispatch: report_findings is universal; domain tools go to store or live
